...
export function ExploreView({ onViewChange, onListingClick }: { onViewChange: (v: ViewState) => void, onListingClick: (l: Listing) => void }) {
  const [activeType, setActiveType] = useState<'car' | 'estate'>('car');
    ...
    <div className="pb-24 w-full h-full overflow-y-auto">
      ...
      <div className="relative h-64 bg-black">
        ...
        <div className="relative z-10 px-4 mt-8">
          ...
          <h3 className="text-xl font-serif text-gold mb-2">
            {activeType === 'car' ? 'Drive excellence.' : 'Extraordinary spaces.'}
          </h3>
          <p className="text-sm text-zinc-300 w-2/3">
            {activeType === 'car' ? 'Explore the world\'s finest cars. Buy or sell with confidence.' : 'Discover the world\'s most exceptional homes and investments.'}
          </p>
        </div>
      </div>
      ...
      <section className="px-4 mt-4">
        ...
        <div className="bg-zinc-100 dark:bg-dark-surface rounded-xl flex items-center p-3 mb-4 border border-zinc-200 dark:border-dark-border">
          ...
          <input 
            type="text" 
            placeholder={activeType === 'car' ? "Search for make, model, or keyword" : "Search by location, property name, or keyword"}
        ...
        <div className="flex gap-2 overflow-x-auto no-scrollbar mb-6">
          <button className="flex-shrink-0 bg-gold/10 border border-gold text-gold px-4 py-2 rounded-full text-sm font-medium flex items-center gap-2">
            <div className="flex gap-0.5"><div className="w-2 h-2 bg-gold"/><div className="w-2 h-2 bg-gold"/></div>
            All {activeType === 'car' ? 'Cars' : 'Estates'}
          </button>
          {['Make', 'Model', 'Price'].map(chip => (
            <button key={chip} className="flex-shrink-0 bg-zinc-100 dark:bg-dark-surface border border-zinc-200 dark:border-dark-border text-zinc-700 dark:text-zinc-300 px-4 py-2 rounded-full text-sm font-medium flex items-center gap-1">
              {chip} <ChevronDown className="w-4 h-4 text-zinc-500" />
            </button>
          ))}
          <button className="flex-shrink-0 bg-zinc-100 dark:bg-dark-surface border border-zinc-200 dark:border-dark-border text-zinc-700 dark:text-zinc-300 px-4 py-2 rounded-full text-sm font-medium flex items-center gap-1">
        ...
        <div className="flex justify-between items-center mb-4">
          <span className="text-sm text-zinc-500">1,248 {activeType === 'car' ? 'Cars' : 'Estates'} Found</span>
