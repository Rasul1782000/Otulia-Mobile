...
export function HomeView({ onViewChange, onListingClick, onCategorySelect }: { onViewChange: (v: ViewState) => void, onListingClick: (l: Listing) => void, onCategorySelect?: (type: Listing['type']) => void }) {
  return (
    <div className="pb-24 w-full h-full overflow-y-auto">
      ...
      <section className="mt-2">
        ...
        <div className="flex overflow-x-auto px-4 gap-4 no-scrollbar pb-4 snap-x snap-mandatory scroll-smooth">
          {categories.map((cat) => (
            <div key={cat.id} className="min-w-[140px] h-48 rounded-xl relative overflow-hidden flex-shrink-0 cursor-pointer snap-start" onClick={() => {
              if (cat.type && onCategorySelect) {
      ...
      <section className="mt-6 px-4">
        ...
        <div className="flex overflow-x-auto gap-4 no-scrollbar pb-4 -mx-4 px-4 snap-x snap-mandatory scroll-smooth">
          {listings.filter(l => l.isFeatured).map(listing => (
            <div key={listing.id} className="min-w-[240px] bg-zinc-100 dark:bg-dark-surface rounded-xl overflow-hidden border border-zinc-200 dark:border-dark-border cursor-pointer flex-shrink-0 snap-center" onClick={() => onListingClick(listing)}>
      ...
      <section className="px-4 mt-6">
        <div className="relative w-full h-32 rounded-xl overflow-hidden flex items-center">
          ...
          <div className="relative z-10 px-6">
            ...
            <button className="bg-gold hover:bg-gold-hover text-white text-xs font-semibold py-2 px-4 rounded-lg transition-colors" onClick={() => onViewChange('add-listing')}>
              List Your Item &rarr;
            </button>
          </div>
        </div>
