  ...
  return (
    <ScrollView style={tw`flex-1 bg-white pb-24`} contentContainerStyle={tw`pb-24`}>
      ...
      <View style={tw`flex-row justify-between items-center px-6 py-4 pt-12 bg-white`}>
        ...
        <View style={tw`flex-col items-center`}>
          <Image 
            ...
            resizeMode="contain"
          />
          <Text style={tw`text-lg tracking-[10px] font-black text-zinc-900 ml-2`}>OTULIA</Text>
        </View>
      ...
      <View style={tw`px-6 mt-8 bg-white`}>
        <Text style={tw`text-4xl text-zinc-900 mb-1 font-serif font-black leading-tight`}>One destination.</Text>
        <Text style={[tw`text-4xl mb-6 italic font-serif font-black leading-tight`, { color: colors.gold }]}>Every luxury.</Text>
      ...
      <View style={tw`mt-2 bg-white`}>
        <View style={tw`flex-row justify-between items-end px-6 mb-6`}>
          <View>
            <Text style={tw`text-2xl font-black text-zinc-900 mb-1`}>Collections</Text>
            <Text style={tw`text-xs font-black text-zinc-500 uppercase tracking-widest`}>Browse by category</Text>
          </View>
          <TouchableOpacity onPress={() => onViewChange('explore')} style={tw`pb-1`}>
            <Text style={[tw`text-xs font-black uppercase tracking-wider`, { color: colors.gold }]}>View All &rarr;</Text>
          </TouchableOpacity>
        </View>
          ...
          {categories.map((cat) => (
            ...
            >
              ...
              <View style={tw`absolute inset-0 p-4 justify-end`}>
                <Text style={tw`text-sm font-black text-white tracking-wider mb-1 uppercase`}>{cat.name}</Text>
                <Text style={tw`text-[10px] font-black text-zinc-100 uppercase tracking-widest`}>{cat.subtitle}</Text>
              </View>
            </TouchableOpacity>
          ))}
        </ScrollView>
      ...
      <View style={tw`mt-10 px-6 bg-white`}>
        <View style={tw`flex-row justify-between items-end mb-6`}>
          <View>
            <Text style={tw`text-2xl font-black text-zinc-900 mb-1`}>Featured</Text>
            <Text style={tw`text-xs font-black text-zinc-500 uppercase tracking-widest`}>Exclusive selections</Text>
          </View>
          <TouchableOpacity onPress={() => onViewChange('explore')} style={tw`pb-1`}>
            <Text style={[tw`text-xs font-black uppercase tracking-wider`, { color: colors.gold }]}>Explore &rarr;</Text>
          </TouchableOpacity>
        </View>
        <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={tw`gap-5 pb-8`}>
          {listings.filter(l => l.isFeatured).map(listing => (
            ...
            >
              ...
              <View style={tw`p-5`}>
                <Text style={tw`font-black text-base mb-1 text-zinc-900`} numberOfLines={1}>{listing.title}</Text>
                <View style={tw`flex-row items-center gap-1 mb-4`}>
                  <Search size={10} color={colors.gold} />
                  <Text style={tw`text-[11px] font-black text-zinc-500 uppercase tracking-wider`}>{listing.location}</Text>
                </View>
                <View style={tw`flex-row items-center justify-between border-t border-zinc-50 pt-4`}>
                  <Text style={tw`font-black text-lg text-zinc-900`}>{listing.currency}{listing.price.toLocaleString()}</Text>
      ...
      <View style={tw`px-6 mt-6 mb-10`}>
        ...
        >
          ...
          <View style={tw`relative z-10 px-6`}>
            <Text style={[tw`text-xl font-black mb-1`, { color: colors.gold }]}>List your luxury.</Text>
            <Text style={tw`text-sm font-black text-white mb-4`}>Reach the right audience.</Text>
            <View style={[tw`py-2 px-5 rounded-full self-start`, { backgroundColor: colors.gold }]}>
              <Text style={tw`text-white text-xs font-black uppercase tracking-widest`}>Get Started</Text>
            </View>
          </View>
        </TouchableOpacity>
      </View>
